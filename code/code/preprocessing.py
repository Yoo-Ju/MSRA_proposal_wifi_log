...
def add_revisit_intention(mpframe2, revisit_interval_thres):
    ...
    del mpframe2['cnt']
    mpframe2.loc[:, 'revisit_intention'] = mpframe2['revisit_intention'].astype(int)


    ### 최근 세달 이내 revisit intention이 없는 moving pattern 제거 
    # mpframe2 = mpframe2.set_index('date_device_id')
    limit_date = max(mpframe2.date) - revisit_interval_thres
    recent3monthvisitors = mpframe2.loc[(mpframe2.date > limit_date) & (mpframe2.revisit_intention == 0)].index
    trajs_excludelast3months = mpframe2.drop(recent3monthvisitors)
