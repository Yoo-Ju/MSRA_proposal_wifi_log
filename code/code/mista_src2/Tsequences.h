...
template<class T> void _myread(T& x);
...
public:
	...
	vector<TProjection*> *generate_projections(bool disk_in, bool disk_out, float max_time_gap);
	...
	H_store *extract_dense_rectangles(float tau, indexing_strategy index_strat);
	H_store *extract_dense_rectangles_brute_force(float tau, indexing_strategy index_strat);
	H_store *extract_dense_rectangles_brute_force2(float tau, indexing_strategy index_strat);
